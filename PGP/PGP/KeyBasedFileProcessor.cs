    ...
    */
    public sealed class KeyBasedFileProcessor
    {
            ...
            string defaultFileName)
        {
            ...
            try
            {
                ...
                if (message is PgpLiteralData)
                {
                    ...
                    if (outFileName.Length == 0) outFileName = defaultFileName;
            ...
            bool withIntegrityCheck)
        {
            ...
            try
            {
                var bytes = PgpExampleUtilities.CompressFile(fileName, CompressionAlgorithmTag.Zip);

                var encGen = new PgpEncryptedDataGenerator(
                    SymmetricKeyAlgorithmTag.Cast5, withIntegrityCheck, new SecureRandom());
                encGen.AddMethod(encKey);

                var cOut = encGen.Open(outputStream, bytes.Length);
